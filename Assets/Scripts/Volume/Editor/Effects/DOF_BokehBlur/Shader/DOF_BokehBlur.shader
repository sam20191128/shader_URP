Shader "URP/Post/Blur/DOF_BokehBlur"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        TEXTURE2D_X_FLOAT(_CameraMotionVectorsTexture);
        SAMPLER(sampler_CameraMotionVectorsTexture);

        TEXTURE2D_X_FLOAT(_CoCTex);
        SAMPLER(sampler_CameraDepthTexture);

        TEXTURE2D_X_FLOAT(_DepthOfFieldTex);
        SAMPLER(sampler_DepthOfFieldTex);
        ...
        half4 DOF_BokehBlur(VertexOutput i) : SV_Target
        {
            ...
            // finalColor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(i.uv0 - duv.xy));
            // finalColor += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(i.uv0 - duv.zy));
            // finalColor += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(i.uv0 + duv.zy));
            // finalColor += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(i.uv0 + duv.xy));
        ...
        // CoC calculation
        half4 FragCoC(VertexOutput i) : SV_Target
        {
            float depth = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, i.texcoordStereo), _ZBufferParams);
            half coc = (depth - _Distance) * _LensCoeff / max(depth, 1e-4);
            return saturate(coc * 0.5 * _RcpMaxCoC + 0.5);
        }

        // Temporal filter
        half4 FragTempFilter(VertexOutput i) : SV_Target
        {
            float3 uvOffs = _MainTex_TexelSize.xyy * float3(1.0, 1.0, 0.0);

            #if UNITY_GATHER_SUPPORTED

                half4 cocTL = GATHER_RED_TEXTURE2D(_CoCTex, sampler_CoCTex, UnityStereoTransformScreenSpaceTex(i.uv0 - uvOffs.xy * 0.5)); // top-left
                half4 cocBR = GATHER_RED_TEXTURE2D(_CoCTex, sampler_CoCTex, UnityStereoTransformScreenSpaceTex(i.uv0 + uvOffs.xy * 0.5)); // bottom-right
            ...
            half coc1 = SAMPLE_TEXTURE2D(_CoCTex, sampler_CoCTex, UnityStereoTransformScreenSpaceTex(i.uv0 - uvOffs.xz)).r; // top
            half coc2 = SAMPLE_TEXTURE2D(_CoCTex, sampler_CoCTex, UnityStereoTransformScreenSpaceTex(i.uv0 - uvOffs.zy)).r; // left
            half coc3 = SAMPLE_TEXTURE2D(_CoCTex, sampler_CoCTex, UnityStereoTransformScreenSpaceTex(i.uv0 + uvOffs.zy)).r; // bottom
            half coc4 = SAMPLE_TEXTURE2D(_CoCTex, sampler_CoCTex, UnityStereoTransformScreenSpaceTex(i.uv0 + uvOffs.xz)).r; // right

            #endif

            // Dejittered center sample.
            half coc0 = SAMPLE_TEXTURE2D(_CoCTex, sampler_CoCTex, UnityStereoTransformScreenSpaceTex(i.uv0 - _TaaParams.xy)).r;
            ...
            float2 motion = SAMPLE_TEXTURE2D(_CameraMotionVectorsTexture, sampler_CameraMotionVectorsTexture, UnityStereoTransformScreenSpaceTex(i.uv0 + closest.xy)).xy;
            half cocHis = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(i.uv0 - motion)).r;
        ...
        half4 FragPrefilter(VertexOutput i) : SV_Target
        {
            ...
            float2 uv0 = UnityStereoTransformScreenSpaceTex(i.uv0 - duv.xy);
            float2 uv1 = UnityStereoTransformScreenSpaceTex(i.uv0 - duv.zy);
            float2 uv2 = UnityStereoTransformScreenSpaceTex(i.uv0 + duv.zy);
            float2 uv3 = UnityStereoTransformScreenSpaceTex(i.uv0 + duv.xy);
        ...
        half4 FragBlur(VertexOutput i) : SV_Target
        {
            //预计算旋转
            float c = cos(2.39996323f);
            float s = sin(2.39996323f);
        ...
        // Postfilter blur
        half4 FragPostBlur(VertexOutput i) : SV_Target
        {
            // 9 tap tent filter with 4 bilinear samples
            const float4 duv = _MainTex_TexelSize.xyxy * float4(0.5, 0.5, -0.5, 0);
            half4 acc;
            acc = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(i.uv0 - duv.xy));
            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(i.uv0 - duv.zy));
            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(i.uv0 + duv.zy));
            acc += SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(i.uv0 + duv.xy));
            return acc / 4.0;
        }
        ...
        // Combine with source
        half4 FragCombine(VertexOutput i) : SV_Target
        {
        ...
        // // Debug overlay
        // half4 FragDebugOverlay(VertexOutput i) : SV_Target
        // {
